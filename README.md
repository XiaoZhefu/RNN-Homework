...
本项目主要在 Conda 环境下运行，所需软件和核心库如下：

| 软件/库    |          版本 | 用途                                       |
| ---------- | ------------: | ------------------------------------------ |
| Python     |       3.10.20 | 执行数据读取、模型训练、模型评估和总控脚本 |
| MATLAB     |        R2024b | 数据清洗辅助验证和结果可视化               |
| PyTorch    |   2.7.0+cu128 | 构建、训练和评估 LSTM/GRU 模型             |
| NumPy      |         2.2.6 | 数组运算和评价指标计算                     |
| Pandas     |         2.3.3 | CSV 数据读取、整理和结果导出               |
| subprocess | Python 内置库 | 在 Python 中调用 MATLAB 脚本               |

若使用 GPU 训练，需要本机 CUDA 与 PyTorch 版本匹配；若不使用 GPU，脚本会自动切换到 CPU。
